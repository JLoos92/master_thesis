...
options=("elmerice_2d folder" "uniform-mesh folder" "standard/test" "Quit")
select opt in "${options[@]}"
do
    case $opt in
        "elmerice_2d folder")
			mkdir -p //volumes/esd01/docs/jloos/data_small/runs_elmerice_2d/Mesh"$a"_"$x""$y"_"$dl" | rsync -ruvt Cluster:/beegfs/work/zxmjf89/Channel_Setup/channel2d //volumes/esd01/docs/jloos/data_small/runs_elmerice_2d/Mesh"$a"_"$x""$y"_"$dl"/
            break
            ;;
        "refined folder")
