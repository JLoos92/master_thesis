...
cutter = ModelRun(250,500,500,0,50).cutter
...
points_cut = vtk_to_numpy(cutter().GetPoints().GetData())
points_cut_1 = vtk_to_numpy(cutter_1().GetPoints().GetData())
velo_cut= vtk_to_numpy(cutter().GetPointData().GetArray(17))
velo_cut= velo_cut[:,0]
...
#zcalc = ((p_w*zcalc)/(p_w-p_i))#-(((p_a-p_i)/(p_i-p_w))* H_a)
npzcalc = np.divide((p_w*zcalc),(p_w-p_i))
hh = zcalc - zs    
...
ctd = CloughTocher2DInterpolator(points,hh)
ctd_values = np.squeeze(ctd.values)

plt.triplot(points_cut[:,0],points_cut[:,1],delaunay.simplices,alpha=0)
...
#plt.scatter(points_cut[:,0],points_cut[:,1],hh)
#plt.show()
