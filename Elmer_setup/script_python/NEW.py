...
depth = vtk_to_numpy(cutter.GetOutput().GetPointData().GetArray('depth'))
...
hull1 = concave_hull.compute(hull1_points,4)

hull2_points = np.delete(line_points2, 0, 1)
hull2 = concave_hull.compute(hull2_points,4)
...
#zs_s = np.divide((p_w*zs),(p_w-p_i))


zs_ind = np.asarray(np.where(zs>0))
zs_ind = zs_ind.transpose().flatten()
zs_s = zs[zs>0]
zb_s = zb[zb<0]
...
x_new = x[zs_ind]
y_new = y[zs_ind]
zs_s = np.divide((p_w*zs_s),(p_w-p_i))
zs_s = zs[zs>0]
new_h = -zs_s-zb_s
hh = new_h - zs_s 
...
# Interpolation for
xx,yy = np.meshgrid(line_points3[:,1],line_points3[:,2])
rbf = Rbf(line_points3[:,1],line_points3[:,2],stress_x,epsilon=2,function = 'gaussian')
zz = rbf(xx,yy)
...
plt.tripcolor(tri,hh,shading='gouraud')
plt.clim(-30,30)
