...
def plot_rms_2d(t1=None,
    ...
    '''
    Function: plot_rms_2d
    ---------------
    This function calculates the RMS for all given channel widths and all three
    domain sizes (regular,extended, wide-extended). A second plot shows the
    channel peak deviation. This function generates 4 subplots where each refer
    to one timestep.
    ...
    # Define rc_params for figure layout
    nrow = 1; ncol = 4;
    ...
    for ax1,num,t in zip(ax.reshape(-1),nums,times): 
        ...
        for width in list_widths_1:
            ...
            # compute hydrostatic deviation for each domain
    ...
    path = str('plots/03_results/05_correlation/')
    fname_pdf= str('allwidths_corr_2d_alldomains'+ '_' + str(t*5) + '.pdf')
    fname_png= str('allwidths_corr_2d_alldomains_250.'+ '_' + str(t*5) + '.png')
