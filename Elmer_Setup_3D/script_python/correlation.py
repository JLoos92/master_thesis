...
runs = [50,75,100,125,150,200,250,300,400,500]
...
for i in runs:
    ...
    hydrostatic_thick = ModelRun(250,i,i,0,200).compute_hydrostatic_thickness()
    ...
    df_sorted_new = df_sorted[(df_sorted['Y']>= y1) & (df_sorted['Y']<= y2)]
    df_sorted_new = df_sorted_new[(df_sorted['X']>= x1) & (df_sorted_new['X']<= x2)]
...
#====================================================================== 
    ...
    df_sorted_noise = df_sorted_noise[(df_sorted_noise['X']>= x1) & (df_sorted_noise['X']<= x2)]
    hd_range_noise = df_sorted_noise.iloc[:,2].values

    df_sorted_noise_2 = df_sorted[(df_sorted['Y']<= y1)]
    df_sorted_noise_2 = df_sorted_noise_2[(df_sorted_noise_2['X']>= x1) & (df_sorted_noise_2['X']<= x2)]
