...
class Plot_scalar_2D():
    
    
    '''
    This class defines 2-D plots for velocity in x and y direction, as well as
    horizontal shear stress.  The figure is splitted into two subplots with two
    timesteps for one channel width.
    '''
    ...
    def __init__(self,
        ...
        prop : int (0 for regular domain)
            timestep for plot   
        scalar : string
            e.g. "velocity_x", "velocity_y" or " sxy"
       ...
       '''                 
        ...
        for ax,num,t in zip(ax1.reshape(-1),nums,times): 
                ...
                elif scalar == str('second_invariant'):
                    cmap = str("RdYlBu")                
                ...
                # Get top and bottom surface with x-coordinates
                upper=ht[2]
                lower=ht[3]
                new_x = ht[0]
        ...
        ax1[0][0].axhline(y=-125,color='y')
        
        ax1[0][0].text(-1500,-124,"A",ha = 'left', va='center',color = 'y')
        ax1[0][0].text(1500,-124,"A'",ha = 'right', va='center', color = 'y')
        ...
        fname_png = str(scalarname + '_dev_2d_' + str(original_width*2) + '.png')
        fname_pdf = str(scalarname + '_dev_2d_' + str(original_width*2) + '.pdf')
